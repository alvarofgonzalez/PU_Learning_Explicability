...
class ImageAuthorshipDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        self.image_embeddings = Tensor(
            pickle.load(
                open("/home/lidia/alvaro/TFG_Alvaro/data/"+self.city+'/tripadimgrest_virgindata_' + self.city +'/IMG_VEC', 'rb')
            )
        )
...
class TripadvisorImageAuthorshipBCEDataset(Dataset):
    def __init__(
        ...
        self.dataframe = pickle.load(
            open(f"/home/lidia/alvaro/TFG_Alvaro/data/{city}/tripadimgrest_virgindata_{city}/{partition_name}_IMG", 'rb')
        )
